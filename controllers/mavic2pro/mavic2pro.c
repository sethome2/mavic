...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    const double roll_input = k_roll_p * CLAMP(roll, -2.0, 2.0) + roll_velocity + roll_disturbance;
    const double pitch_input = k_pitch_p * CLAMP(pitch, -2.0, 2.0) + pitch_velocity + pitch_disturbance;
